  ...
  return !isDelete ? (
    ...
    <Tip lbl="Сэргээх">
      <button
        ...
        // onClick={onClick}
      >
        <FaTrashRestoreAlt size={16} className="text-white" />
      </button>
    </Tip>
  );
};
...
export const ImageCount = ({ onClick, children }) => (
  <Tip lbl="Зурагны тоо">
    <button
      className="flex items-center justify-center w-10 h-6 gap-1 text-white rounded-full bg-gray-600/80 backdrop-blur-sm md:w-12"
      onClick={onClick}
    >
